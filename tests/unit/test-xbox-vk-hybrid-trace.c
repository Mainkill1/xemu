...
static void test_failure_flushes_pending_frame(void)
{
    g_autofree char *path = new_trace_path();
    PGRAPHVkHybridTrace *trace =
        pgraph_vk_hybrid_trace_open(path, UINT64_MAX);
    ...
    pgraph_vk_hybrid_trace_record(
        trace, VK_HYBRID_TRACE_PIPELINE_PROBE, 1, 7, 8, 9,
        10, 11, 12, 13);
    pgraph_vk_hybrid_trace_failure(trace, -4, 1);

    /* A crash would skip close(), so the failure path must flush the file. */
    g_assert_true(g_file_get_contents(path, &contents, NULL, NULL));
    g_assert_nonnull(strstr(contents, "failure,0,1,-4,1,1,0"));
    g_assert_nonnull(strstr(contents, "6,0,1,"));
...
int main(int argc, char **argv)
{
    ...
    g_test_add_func("/xbox/vk/hybrid-trace/failure-flush",
                    test_failure_flushes_pending_frame);
    return g_test_run();
}
