...
#undef NV2A_MAX_BATCH_LENGTH
#define NV2A_MAX_BATCH_LENGTH 16
#define LOG_GUEST_ERROR 1
static unsigned dropped_packets;
#define qemu_log_mask(...) (dropped_packets++)
...
typedef struct PGRAPHState {
    ...
    unsigned int draw_arrays_count[16];
...
code += function(pgraph, "static void pgraph_drop_oversized_array_packet(")
...
int main(void) {
                ...
                for (unsigned inc = 0; inc < 2; inc++) {
                    ...
                    for (unsigned pending = 0; pending < 2; pending++) {
                        ...
                        if (pending && m != 2) {
                            ...
                            pg.draw_arrays_count[0] = NV2A_MAX_BATCH_LENGTH;
                        } else {
                            pg.inline_elements_length = NV2A_MAX_BATCH_LENGTH;
                            pg.inline_array_length = NV2A_MAX_BATCH_LENGTH;
                        }
                        PGRAPHState before = pg;
                        scalar_calls = 0;
                        dropped_packets = 0;
                        consumed = 0;
                        pgraph_method_non_inc(scalar, &d, &pg, 0, methods[m],
                        ...
                        assert(scalar_calls == 0);
                        assert(!memcmp(&pg, &before, sizeof(pg)));
                    }
                }
            }
        }
    ...
    puts("PASS: 24 packet modes, 48 capacity refusals and On/Off reservation fallbacks");
}
'''
