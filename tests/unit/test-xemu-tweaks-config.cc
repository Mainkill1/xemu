...
int main()
{
    ...
    for (unsigned i = 0; i < XEMU_TWEAK_COUNT; i++) {
        ...
        assert(xemu_tweak_enabled(tweak) == restart);
    }
    xemu_tweaks_apply(true);
    for (unsigned i = 0; i < XEMU_TWEAK_COUNT; i++) {
        assert(!xemu_tweak_enabled(static_cast<XemuTweak>(i)));
    ...
    assert(!tweaks->child(keys[0])->data.boolean.val);
    for (unsigned i = 1; i < sizeof(keys) / sizeof(keys[0]); i++) {
        assert(tweaks->child(keys[i])->data.boolean.val);
    }
    config_tree.free_allocations(&g_config);
    config_tree.store_to_struct(&g_config);
    xemu_tweaks_apply(true);
    ...
    for (unsigned i = 1; i < XEMU_TWEAK_COUNT; i++) {
        assert(xemu_tweak_enabled(static_cast<XemuTweak>(i)));
    }
