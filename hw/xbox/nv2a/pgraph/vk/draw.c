                                ...
                                VkDeviceAddress alignment)
{
    ...
    if (!pgraph_vk_buffer_has_space_for(pg, index, size, alignment)) {
        ...
        /*
         * Finishing submits the staged data and resets the transient offset.
         * Reuse that storage unless this individual allocation still exceeds
         * the empty buffer. Growing from the pre-finish offset needlessly
         * recreated both buffers on every wrap.
         */
        if (!pgraph_vk_buffer_has_space_for(pg, index, size, alignment)) {
