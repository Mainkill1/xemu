...
void pgraph_vk_process_fallback_families(PGRAPHState *pg)
{
    ...
    while (visited < ARRAY_SIZE(r->fallback_family_requests) &&
        ...
        if (status == PGRAPH_VK_HYBRID_PIPELINE_ACCEPTED ||
            status == PGRAPH_VK_HYBRID_PIPELINE_UNSUPPORTED_RECIPE ||
...
static bool create_pipeline(PGRAPHState *pg)
{
    ...
    if (hybrid) {
        ...
        if (!specialized.shader || !specialized.pipeline) {
            ...
            /* A completed family can serve a new combiner even though its
             * full-state binding has not been seen before. This creates only
             * binding metadata from already-ready modules. */
            if (!fallback.shader && !specialized.shader &&
                controls_supported &&
                pgraph_vk_prepare_binding_from_ready_modules(
                    pg, &requested_state,
        ...
        if (selected == PGRAPH_VK_EXECUTION_SPECIALIZED) {
            ...
            route = pgraph_vk_hybrid_choose_uncovered_route(
                ...
                controls_supported);
            queue_fallback_family =
                route == PGRAPH_VK_FRAGMENT_SPECIALIZED &&
                controls_supported && !fallback.shader;
            ready_shader = NULL;
            ready_pipeline = NULL;
    ...
    if (queue_fallback_family) {
        PipelineKey fallback_key;
        init_pipeline_key_for_state(pg, &requested_state,
                                    PGRAPH_VK_FRAGMENT_UBERSHADER,
                                    &fallback_key);
        pgraph_vk_fallback_family_enqueue(
            r->fallback_family_requests,
            ARRAY_SIZE(r->fallback_family_requests),
            &fallback_key, &requested_state);
    }
...
static bool can_version_vertex_draw(PGRAPHState *pg, uint32_t num_vertices)
{
    ...
    for (int i = 0; i < r->num_active_vertex_binding_descriptions; i++) {
        ...
        if (!dma || addr > vram_size || attr->offset > dma_len ||
            !pgraph_vk_vertex_version_span_fits(
                num_vertices, stride, element_bytes, vram_size - addr) ||
            !pgraph_vk_vertex_version_span_fits(
                num_vertices, stride, element_bytes, dma_len - attr->offset) ||
                                       ...
                                       size_t page_count, bool stale)
{
    ...
    for (size_t page = first_page; page < first_page + page_count; page++) {
        if (r->vertex_ram_stale_pages[page] == stale) {
            continue;
        }
        r->vertex_ram_stale_pages[page] = stale;
        if (stale) {
            r->vertex_ram_stale_page_count++;
        } else {
            assert(r->vertex_ram_stale_page_count);
            r->vertex_ram_stale_page_count--;
        }
    }
}

static bool sync_vertex_ram_buffer(PGRAPHState *pg, uint32_t num_vertices)
{
    NV2AState *d = container_of(pg, NV2AState, pgraph);
    PGRAPHVkState *r = pg->vk_renderer_state;
                                                    ...
                                                    bool version_vertex_data)
{
    ...
    for (int attr_id = 0; attr_id < NV2A_VERTEXSHADER_ATTRIBUTES; attr_id++) {
        ...
        if (offset_valid && stride_valid && !version_vertex_data) {
            continue;
        }
...
static bool pgraph_vk_flush_draw_internal(NV2AState *d)
{
    ...
    if (pg->draw_arrays_length) {
        ...
        bool version_vertex_data = sync_vertex_ram_buffer(pg, max_element);
        ...
        VertexBufferRemap remap = remap_unaligned_attributes(
            pg, max_element, version_vertex_data);
        ...
        copy_remapped_attributes_to_inline_buffer(pg, remap, min_element,
                                                  max_element);
        if (version_vertex_data && r->perf.enabled) {
            r->perf.vertex_version_draw_count++;
            r->perf.vertex_version_bytes += remap.buffer_space_required;
        }
        ...
        bool version_vertex_data = sync_vertex_ram_buffer(pg,
                                                          max_element + 1);
        pgraph_vk_refresh_vertex_inline_values_after_sync(pg,
                                                          provoking_element);
        VertexBufferRemap remap = remap_unaligned_attributes(
            pg, max_element + 1, version_vertex_data);
