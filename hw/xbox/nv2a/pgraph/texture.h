 ...
 * Border texels are part of the NV2A storage layout for non-linear textures;
 * decoded Vulkan dimensions are deliberately kept separate from this source
 * layout.
 */
    ...
    size_t *length)
{
    ...
    if (!length || shape.color_format >= ARRAY_SIZE(kelvin_color_format_info_map) ||
